...
import { getByStatusDeal, setActiveDeal } from "../../../services/DealService";
...
function HomeDeal() {
  ...
  useEffect(() => {
    ...
    getByStatusDeal(trang, bd, kt, status)
      .then((response) => {
          setData(response.content);
          setTotalPages(response.totalPages || 1);
      })
      .catch((error) => {
        toast.error("Không thể tải dữ liệu Deal");
      });
      ...
      <div className="bg-white shadow-md rounded-xl p-4">
        <div className="flex justify-between items-center mb-4">
          <p className="text-left   mt-4  bg-white  rounded-md flex">
            ...
            <h2 className="text-xl font-semibold">Tổng quan Deal của Shop</h2>
          </p>
          <button
          ...
          <table className="w-full text-sm text-left border">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                ...
                <th className="p-2 border">Số lượng sản giảm giá</th>
                ...
                data.map((d) => (
                  <tr >
                    ...
                    <td className={`p-2 border ${isPastTime(d.thoiGianNgung)?"pointer-events-none bg-gray-200":null}`}>
                      <button
                        onClick={() => {
                          setActiveDeal(d.id, !d.conSuDung)
                            .then(() => {
                              if (!d.conSuDung) {
                                toast.success("Kích hoạt Deal thành công");
                              } else {
                                toast.success("Vô hiệu Deal thành công");
                              }
