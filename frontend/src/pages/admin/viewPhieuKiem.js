...
import { getUpdatePhieuKiem, saveKhauHao, updatePhieuKiem } from "../../services/PhieuNhapService";
...
function ViewPhieuKiem() {
    ...
    const [openConflictModal, setOpenConflictModal] = useState(false); // New state for conflict modal
    const [conflictPhieuKiemId, setConflictPhieuKiemId] = useState(null); // Store conflicting phieuKiemDaCo
    ...
    const handleAddItem = (item) => {
        // Check if phieuKiemDaCo is not 0 and does not match the current id
        if (item.phieuKiemDaCo !== 0 && item.phieuKiemDaCo !== parseInt(id)) {
            setConflictPhieuKiemId(item.phieuKiemDaCo);
            setOpenConflictModal(true); // Open conflict modal
            return;
        }

        // Existing logic if no conflict
    ...
    return (
        <div className="p-4 space-y-6">
            ...
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold w-fit p-2 rounded-md shadow-md border"> <i class="fa-solid fa-file-pen mr-1 text-green-900"></i> Thông tin phiếu kiểm</h2>
                        ...
                        {dataUpdate?.danhSachChiTiet?.length === 0 ? (
                            ...
                            dataUpdate?.danhSachChiTiet?.map((item, index) => (
                                <tr key={item.id} className={`text-center ${loai?.some(d => d?.id === item.id) ? "bg-gray-100" : ""}`}>
                                    ...
                                    <td className="border-t border-gray-400p-2 p-2 flex justify-center items-center">
                                       {
                                        item?.danhSachHoaDonBiHuy?.length>0?<span
                                            onClick={() => {
                                                pickk.current = item;
                                            ...
                                            className="relative mt-2  ml-2 mr-2 cursor-pointer flex items-center justify-center size-6"
                                        >
                                            {kiemTraSoLuong(
                                                item.result,
                                                item.danhSachHoaDonBiHuy,
            ...
            {openGhiChu && (
                <Modal setOpen={setOpenGhiChu}>
                    <p className="font-semibold mb-2"><i className="fa-solid fa-notes-medical p-2  text-green-900 rounded-md shadow-md"></i> Ghi chú hao hụt hàng hóa:</p>
                    <textarea
                        defaultValue={dataUpdate.danhSachChiTiet[pick.current]?.ghiChu || ""}
                        className="w-[400px] h-[200px] mt-3 border border-gray-300 outline-none pl-2 pt-1 rounded pointer-events-none"
            ...
            {openGhiChu2 && (
                <Modal setOpen={setOpenGhiChu2}>
                    <div className="mb-1">
                        <i className="ml-4 fa-solid fa-triangle-exclamation text-green-900 p-2 rounded-md shadow-md rounded-md border"></i>
                        <strong><span className="ml-2">Danh sách hóa đơn được chọn xóa
                            </span></strong>
                    </div>
                        ...
                        <table className="min-w-full table-auto border-collapse border border-gray-300 rounded shadow-md">
                            ...
                            <tbody>
                                {pickk.current?.result?.map((j, index) => (
                                    <tr key={index} className="text-center hover:bg-gray-50 transition duration-150">
                                        ...
                                        <td className="px-4 py-2 border">
                                            <input
                                                type="checkbox"
                                                className="w-5 h-5 accent-green-900 pointer-events-none"
                ...
                <Modal setOpen={setOpenXacNhanTaoPhieu}>
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <button
                            ...
                            onClick={() => {
                                console.log(dataUpdate?.danhSachChiTiet);
            ...
            {/* New Modal for conflicting phieuKiemDaCo */}
            {openConflictModal && (
                   ...
                   <div className="w-[260px]">
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <button
                            className="px-4 py-1 bg-green-800 text-white rounded hover:bg-green-700"
                            onClick={() => {
                                window.open(`/employee/khauhao/${conflictPhieuKiemId}`, '_blank'); // Open in new tab
