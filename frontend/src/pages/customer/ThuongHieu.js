...
import { getByCondition, getInfo } from "../../services/ThuongHieuService";
...
const [d,setD]=useState()
    ...
    useEffect(()=>{
        getInfo(id).then(d=>{setD(d)}).catch();
    },[])
    ...
    useEffect(() => {
        getViewProduct({ ...filter, category: categoryId, tenSanPham: tenSanPham },0).then(data => {
            setListProduct(data.data)
            setTongPhanTu(data.totalElement)
            setTongSoTrang(data.totalPage)
        })
    }, [filter, tenSanPham, categoryId])
    return (
        <div>
            <div className="flex justify-center">
                <img className="w-full p-6" src={d?.anhNen}></img>
            </div>
            <style>
                {`
                ...
                <div className="basis-4/5">
                    ...
                    <div className="flex justify-start items-center">
                        ...
                        <button
                            ...
                            className={`ml-3 transition-all duration-150 pl-2 pr-2 ${filter.orderBy === "banChay"
                                ? "text-green-900 border-b-2 border-green-900"
                                : "text-gray-700 hover:text-green-900"
                                }`}
                        >
