...
function KhauHao() {
    ...
    const [dataUpdate, setDataUpdate] = useState({ danhSachChiTiet: [], daXacNhan: false });
    ...
    useEffect(() => {
        if (maVach.trim()) {
            getByMaVachForPhieuKiemV2(maVach, true, 1)
        ...
        <div className="p-4 space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold w-fit p-2 rounded-md shadow-md border">
                    <i className="fa-solid fa-file-pen mr-1 text-green-900"></i> Tạo phiếu hao hụt
                </h2>
                <div className="flex items-center gap-6 text-sm text-gray-700">
                    <span className="p-2 shadow-md rounded-md border text-green-900">Nhân viên: <strong>{nhanVien}</strong></span>
                    <span className="p-2 shadow-md rounded-md border text-green-900">
                        Ngày: {dinhDangNgay(new Date())}
                    </span>
            ...
            {openXacNhanTaoPhieu && (
                <Modal setOpen={setOpenXacNhanTaoPhieu}>
                    <h2 className="text-lg font-semibold mb-3">Xác nhận tạo phiếu hao hụt?</h2>
                    <p className="mb-4">Bạn có chắc chắn muốn tạo phiếu với <strong>{dataUpdate?.danhSachChiTiet.length}</strong> sản phẩm?</p>
                    <div className="flex justify-end gap-3">
                        ...
                        <button
                            ...
                            onClick={() => {
                                saveKhauHao(dataUpdate?.danhSachChiTiet)
                                    .then((d) => {
